...
void imageCallback(const sensor_msgs::ImageConstPtr& msg){
    ...
    cv::GaussianBlur(img_roi, img_roi, cv::Size(9, 9), 2);
        
    int threshType;
    threshType = black ? (cv::THRESH_BINARY_INV | cv::THRESH_OTSU) : (cv::THRESH_BINARY | cv::THRESH_OTSU);
    
    cv::threshold(img_roi, img_roi, 0, 255, threshType);
        
    
    // Opening (reduces noise)
    cv::Mat kernel(5, 5, CV_8UC1, 1);
    cv::morphologyEx(img_roi, img_roi, cv::MORPH_OPEN, kernel);

    // find contours
    std::vector<cv::Vec4i> hierarchy;
    cv::findContours(img_roi,contours, hierarchy, cv::RETR_LIST, cv::CHAIN_APPROX_SIMPLE);
